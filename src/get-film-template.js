...
const getFilmTemplate = (data) => {
  ...
  template.innerHTML = `
    <article class="film-card ${data.extra ? `film-card--no-controls` : ``}">
      ...
      <p class="film-card__description ${data.extra ? `visually-hidden` : ``}">${data.description}</p>
      <button class="film-card__comments">${data.comments} comments</button>
      <form class="film-card__controls ${data.extra ? `visually-hidden` : ``}">
